	...
	function content_inp($data)
	{
		...
		if($data['action'] == 'insert'){
			...
			$query = "INSERT INTO  
						...
						{$this->prefix}_news_content (title,
                                                lid,thumbnail_image,slider_image,url,sourceurl,filesize,can_save,tags,
                                                lang_id,menuId,parentid,brief,content,image,file,categoryid,articletype,
												created_date,posted_date,expired_date,authorid,n_status)
					VALUES
						('".$data['contentTitle']."',
                        '0','','','','','0','0','',
                        '".$data['langID']."','".$data['menuId']."','".$data['parentMenu']."','".$data['brief']."','".$data['content']."','".$data['image']."'
