<?php
class mcontactus extends Database {
	
	var $prefix = "uzbemb";
	function contactus_inp($data)
	{
		...
		if($data['action'] == 'insert'){
			...
			$query = "UPDATE {$this->prefix}_contactus
						SET 
	...
	function get_contactus($n_stats=null)
	{
		$query = "SELECT * FROM {$this->prefix}_contactus WHERE n_stats != '2' ORDER BY posted_date DESC";
    ...
    function get_contactus_filter($categoryid=null,$articletype=null,$type=1)
	{
		$query = "SELECT * FROM {$this->prefix}_contactus WHERE n_stats = '1' AND categoryid = '{$categoryid}' AND articleType = '{$articletype}' OR n_status = '0' AND categoryid = '{$categoryid}' AND articleType = '{$articletype}' ORDER BY created_date DESC";
	...
	function get_contactus_id($id)
	{
		$query = "SELECT * FROM {$this->prefix}_contactus WHERE id= {$id}";
    ...
    function topical_del($id)
	{
		...
		foreach ($id as $key => $value) {
			...
			$query = "DELETE FROM {$this->prefix}_contactus WHERE id = '{$value}'";
    ...
    function getcontactus(){
        $query = "SELECT * FROM {$this->prefix}_contactus";
