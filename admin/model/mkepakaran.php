...
// pr($query);
	...
	function insertDataLang($data, $debug)
	{
	  
	  $id = $data['id'];
	  $sql = array(
		      'table' => "{$this->prefix}_news_content",
		      'field' => "created_date, n_status",
		      'value' => "'2014-10-10', '1'",
		      );
		      
	  $res = $this->lazyQuery($sql, $debug);
	  $latest_id = $this->insert_id();
	  
	  $sql = array(
		      'table' => "{$this->prefix}_news_content_repo",
		      'field' => "title, description, otherid",
		      'value' => "'test', 'nhcsajcsaj', $latest_id",
		      );
	...
	function getDataLang($data, $debug)
	{
	  
	  $id = $data['id'];
	  $sql = array(
		      'table' => "{$this->prefix}_news_content AS c, {$this->prefix}_news_content_repo AS r",
		      'field' => "r.*",
		      'condition' => "c.id = {$id}",
		      'limit' => "",
