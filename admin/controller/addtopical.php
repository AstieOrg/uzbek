...
class addtopical extends Controller {
	...
	//pr($menuId); exit;		
		...
		/* folder: topical file:addcontent.html */
			   		...
			   		if(isset($x) && count($x) != 0)
			   		{
						...
						if(!empty($_FILES)){
							if($_FILES['file_image']['name'] != ''){
								...
								$icon = uploadFile('file_icon','news','image');
	...
	public function trash(){
		...
		$data = $this->models->get_topical_trash(1);
		if ($data){
		...
		return $this->loadView('topical/viewtrash');

	}
	
	
	public function topicalrest(){

		global $CONFIG;
		
		$data = $this->models->topical_restore($_POST['ids']);
