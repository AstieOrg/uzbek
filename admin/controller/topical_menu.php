...
class topical_menu extends Controller {
	...
	public function loadmodule()
	{
		...
		$this->models = $this->loadModel('mtopicalmenu');
	...
	public function index(){
		...
		return $this->loadView('topical_menu/list_menu');
	...
	public function addmenu(){
		...
		$this->view->assign('active','active');
		$menuList = $this->getmenuModels->getMenuParent(2);
        ...
        if(!$menuList){
            ...
            $this->view->assign('parentselected',$parentselected);
            $this->view->assign('parentOption', $parentOption);
        }
		...
		return $this->loadView('topical_menu/inputmenu');
	}
	...
	public function menuinp(){
		...
		if(isset($_POST)){
            ...
            $redirect = $CONFIG['admin']['base_url'].'topical_menu';
	...
	public function menudel($action='delete'){
		...
		echo "<script>alert('".$message."');window.location.href='".$CONFIG['admin']['base_url']."topical_menu/'</script>";
